    ...
    unsafe fn new(event_loop: &winit::event_loop::EventLoopWindowTarget<()>) -> Self {
        ...
        let window = window.take().unwrap_or_else(|| {
            glutin_winit::finalize_window(event_loop, winit_window_builder.clone(), &gl_config)
                .expect("failed to finatilize window")
        });
        ...
        let surface_attribute =
            glutin::surface::SurfaceAttributesBuilder::<glutin::surface::WindowSurface>::new()
            .build(window.raw_window_handle(), width, height);

        let gl_surface = unsafe {
            gl_display
                .create_window_surface(&gl_config, &surface_attribute)
                .unwrap()
        };
        ...
        let mut redraw = || {
            ...
            *control_flow = if quit {
...

            {
                ...
                //draw things behind the egui
                egui_glow.paint(gl_window.window());

                //draw things on top of egui
